...
func (s *Suite) TestHouseholdMember() {
	...
	s.Run("Update a household member", func() {
		...
		s.decodeResponse(apiResponse, &updatedMember)

		s.Equal(member, updatedMember)
		deletableMemberId = updatedMember.Id
	})

	s.Run("Update a non-existant household member", func() {
		member := s.createTestHouseholdMember()
		member.Id = "0"
	...
	s.Run("Delete a household member", func() {
		...
		s.assertHttpError(apiResponse, http.StatusNotFound, domain.ErrMemberNotFound.Error())
	})

	s.Run("Delete a household member that doesn't exist", func() {
	...
	s.Run("Delete a household member with active accounts", func() {

		apiResponse, err := s.deleteHouseholdMember("1") // ID coming from mock data
		s.handleErr(err, "error while making request")

		s.assertHttpError(apiResponse, http.StatusBadRequest, domain.ErrMemberHasActiveAccounts.Error())
	})
	s.Run("Activate a household member that doesn't exist", func() {
		nonExistentId := "non-existent-id"
	...
	s.Run("Activate a household member that is already active", func() {
		...
		s.assertHttpError(apiResponse, http.StatusBadRequest, domain.ErrMemberAlreadyActive.Error())
	})

	s.Run("Successfully activate an inactive household member", func() {
		// Create a household member
		member := s.createTestHouseholdMember()

		// First deactivate the member
		deactivateResponse, err := s.deactivateHouseholdMember(member.Id)
		s.handleErr(err, "error while deactivating member")
		s.Equal(http.StatusNoContent, deactivateResponse.StatusCode)
		...
		s.handleErr(err, "error while getting member")
		s.Equal(http.StatusOK, getResponse.StatusCode)

		var activatedMember openapi.HouseholdMember
		s.decodeResponse(getResponse, &activatedMember)
		s.True(*activatedMember.Active, "Member should be active after activation")
	})
	s.Run("Deactivate a household member that doesn't exist", func() {
		nonExistentId := "0"
	...
	s.Run("Deactivate a household member that is already inactive", func() {
		// Create a household member
		member := s.createTestHouseholdMember()
		...
		s.assertHttpError(apiResponse, http.StatusBadRequest, domain.ErrMemberAlreadyInactive.Error())
	})

	s.Run("Successfully deactivate an active household member", func() {
		// Create an active household member
		member := s.createTestHouseholdMember() // This creates an active member by default
		...
		s.decodeResponse(getResponse, &deactivatedMember)
		s.False(*deactivatedMember.Active, "Member should be inactive after deactivation")
	})

	s.Run("List all household members", func() {
		...
		s.handleErr(err, "error while updating member")
		...
		s.handleErr(err, "error while listing household members")

		// Should return at least the members we created (plus any from mock data)
		s.NotNil(memberList.Members)
		s.GreaterOrEqual(len(*memberList.Members), 2) // At least 2 active members
	...
	s.Run("List household members filtered by role", func() {
		...
		memberList, err := s.listHouseholdMembers(params)
		s.handleErr(err, "error while listing household members by role")

		s.NotNil(memberList.Members)
		s.Greater(len(*memberList.Members), 0)

		// Verify all returned members have the parent role
		for _, member := range *memberList.Members {
			s.Equal("parent", member.Role)
		}
		...
		memberList, err = s.listHouseholdMembers(params)
		s.handleErr(err, "error while listing household members by child role")
		...
		for _, member := range *memberList.Members {
			s.Equal("child", member.Role)
		}
	})

	s.Run("List household members filtered by active status", func() {
		...
		memberList, err := s.listHouseholdMembers(params)
		s.handleErr(err, "error while listing active household members")
		...
		for _, member := range *memberList.Members {
			s.True(*member.Active, "All members should be active")
		}
		...
		memberList, err = s.listHouseholdMembers(params)
		s.handleErr(err, "error while listing inactive household members")
		...
		for _, member := range *memberList.Members {
			s.False(*member.Active, "All members should be inactive")
		}
	})

	s.Run("List household members with combined filters", func() {
		...
		memberList, err := s.listHouseholdMembers(params)
		s.handleErr(err, "error while listing active parent household members")
	...
	s.Run("List household members with non-existent role", func() {
		nonExistentRole := "non-existent-role"
		params := openapi.ListHouseholdMembersParams{
			Role: &nonExistentRole,
		}
		memberList, err := s.listHouseholdMembers(params)
		s.handleErr(err, "error while listing household members with non-existent role")

		s.NotNil(memberList.Members)
		s.Equal(0, len(*memberList.Members), "Should return empty list for non-existent role")
	})

	s.Run("List household members returns proper structure", func() {
		...
		s.NotNil(memberList.Members)
		s.IsType(&[]openapi.HouseholdMember{}, memberList.Members)
...
func (s *Suite) createHouseholdMember(householdMemberReq *openapi.HouseholdMemberRequest) (openapi.HouseholdMember, error) {
	...
	s.decodeResponse(apiResponse, &member)

	return member, nil
}

func (s *Suite) listHouseholdMembers(params openapi.ListHouseholdMembersParams) (openapi.HouseholdMemberList, error) {
	req, errReq := http.NewRequestWithContext(
		s.ctx,
		http.MethodGet,
		"http://localhost:9091/household-members",
	...
	s.decodeResponse(apiResponse, &members)

	return members, nil

}

func (s *Suite) getHouseholdMember(id string) (*http.Response, error) {
	req, errReq := http.NewRequestWithContext(
		s.ctx,
		http.MethodGet,
...
func (s *Suite) updateHouseholdMember(member *openapi.HouseholdMember) (*http.Response, error) {
...
func (s *Suite) deleteHouseholdMember(id string) (*http.Response, error) {
	req, errReq := http.NewRequestWithContext(
		s.ctx,
		http.MethodDelete,
		"http://localhost:9091/household-members/"+id,
...
func (s *Suite) deactivateHouseholdMember(id string) (*http.Response, error) {
	req, errReq := http.NewRequestWithContext(
		s.ctx,
		http.MethodPatch,
...
func (s *Suite) activateHouseholdMember(id string) (*http.Response, error) {
	req, errReq := http.NewRequestWithContext(
		s.ctx,
		http.MethodPatch,
