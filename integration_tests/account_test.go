...
func (s *Suite) TestAccount() {
	...
	s.Run(
		...
		func() {
			...
			s.Equal(
				...
				createdAccount.AccountInformation,
			)
			s.Equal(
	...
	s.Run(
		"Create account with inactive member",
		func() {
			householdMember := s.createTestHouseholdMember()
			...
			s.assertHttpError(
				...
				domain.ErrMemberInactive.Error(),
			)
		},
	)
	...
	s.Run(
		...
		func() {
			...
			s.testAccountCreationError(
				account,
				http.StatusBadRequest,
				domain.ErrInvalidCurrency.Error(),
			)
		},
	)
	...
	s.Run(
		"Delete an account with transactions",
		func() {
	...
	s.Run(
		"Check if account can be deleted OK",
		func() {
	...
	s.Run(
		"Check if account can be deleted - false due having transactions",
		func() {
	...
	s.Run(
		...
		func() {
			...
			s.assertHttpError(
				apiResponse,
				http.StatusBadRequest,
				domain.ErrAccountAlreadyInactive.Error(),
			)
		},
	)

	s.Run(
		"Deactivate an account that does not exist",
		func() {
	...
	s.Run(
		...
		func() {
			...
			s.assertHttpError(
				apiResponse,
				http.StatusBadRequest,
				domain.ErrAccountAlreadyActive.Error(),
			)
		},
	)

	s.Run(
		"Reactivate an account that does not exist",
		func() {
	...
	s.Run(
		...
		func() {
			householdMember := s.createTestHouseholdMember()
			accountRequest := s.createTestAccountRequest(
				&householdMember,
				"150",
			)
	...
	s.Run(
		"Update account with invalid data",
		func() {
			householdMember := s.createTestHouseholdMember()
			account := s.createTestAccountRequest(
				&householdMember,
			...
			s.assertHttpError(
				apiResponse,
				http.StatusBadRequest,
				port.ErrInvalidDataFormat.Error(),
			)
		},
	)
	...
	s.Run(
		...
		func() {
			apiResponse := s.listAccountRequest(
				openapi.ListAccountsParams{
					Type:     nil,
					Currency: nil,
					Active:   nil,
	...
	s.Run(
		"List accounts with currency filter",
		func() {
...
func (s *Suite) testAccountCreationError(
	account *openapi.AccountRequest,
	expectedStatus int,
...
func (s *Suite) makeAccountRequest(account *openapi.AccountRequest) (
	*http.Response,
	error,
) {
...
func (s *Suite) checkCanDeleteAccount(id string) (
	*http.Response,
	error,
) {
	req, err := http.NewRequestWithContext(
...
func (s *Suite) deactivateAccountRequest(id string) (
	*http.Response,
	error,
) {
	req, err := http.NewRequestWithContext(
...
func (s *Suite) activateAccountRequest(id string) (
	*http.Response,
	error,
) {
	req, err := http.NewRequestWithContext(
...
func (s *Suite) getAccountRequest(id string) (
	*http.Response,
	error,
) {
	req, err := http.NewRequestWithContext(
...
func (s *Suite) createTestAccountRequest(
	owner *openapi.HouseholdMember,
	currency string,
) *openapi.AccountRequest {
